(ns robinhood.main
  "Kotodama WASM entrypoint for the Robinhood clean-room actor (L4) — Clojure port.
  ...
  facts (`robinhood.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "robinhood")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"robinhood.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "robinhood-compat" :tier tier :entities entities} 200])
...
(defn start! [] :robinhood-compat/ready)
